...
homeFolder = "/home/x/specialty/marketplace"
...
def main():
    ...
    emptyFolders = []
    for folder2 in os.scandir(homeFolder):
        xpath = os.path.join(homeFolder, folder2.name)
        xhtmlPath = os.path.join(xpath, "HTML")
        ximagesFolderPath = os.path.join(xpath, "Images")
        if not os.path.exists(ximagesFolderPath):
            print("no images folder made for {}".format(xpath))
            pass
        else:
            if not os.listdir(ximagesFolderPath):
                emptyFolders.append(folder2.name)


    for emptyFolder in emptyFolders:
        print(emptyFolder)
        initPath = os.path.join(homeFolder, emptyFolder)
        ...
        if not os.path.exists(imagesFolderPath):
            print("error, images path does nto exist for {}".format(initPath))
        else:
            searchName = emptyFolder.lower()
